...
export const SwitchBombero = ({
  ...
  const { nombres, apellidos, cargo, servicio, imagen, cuartel, maquinasAManejar, especialidad } = bombero;
    ...
    <Container>
      ...
      >
        <Col xs={12} sm={12} md={4} xl={4} xxl={4}>
          <Card className="mr-2 mt-4" style={{
            margin: '0 auto', border: cuartel?.nombre.toUpperCase() === 'PRIMERA' ? '3px solid blue' :
            ...
            <Card.Body className="cuerpo-card">
              ...
              <>{especialidad === 'MAQUINISTA' && maquinas.map(maquinaCheck => (
                <Form.Check
                  ...
                  checked={maquinasAManejar?.find(maquina => maquina?._id === maquinaCheck._id) ? true : false}
                />
              ))}</>
            </Card.Body>
            <Card.Body
